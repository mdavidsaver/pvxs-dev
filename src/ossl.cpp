...
namespace std {
template<>
struct default_delete<OSSL_LIB_CTX> {
...
template<>
struct default_delete<X509> {
    inline void operator()(X509* fp) { if(fp) X509_free(fp); }
};
template<>
struct default_delete<STACK_OF(X509)> {
...
epicsThreadOnceId OSSLGbl_once = EPICS_THREAD_ONCE_INIT;
...
void OSSLGbl_init(void*)
{
    owned_ptr<OSSL_LIB_CTX> ctx(__FILE__, __LINE__, OSSL_LIB_CTX_new());
...
{
    impl::threadOnce(&OSSLGbl_once, OSSLGbl_init);
...
//    assert(!SSL_CTX_get_keylog_callback(ctx.ctx));
    ...
    if(!conf.tls_keychain_file.empty()) {
        ...
        impl::owned_ptr<EVP_PKEY> key;
        impl::owned_ptr<X509> cert;
        impl::owned_ptr<STACK_OF(X509)> CAs(__FILE__, __LINE__, sk_X509_new_null());
     ...
     *   TLS 1.3 does not support for session renegotiation.
     ...
     */
    SSL_CTX_set_verify(ctx.ctx, SSL_VERIFY_PEER|SSL_VERIFY_CLIENT_ONCE,
                       &ossl_verify);
...
std::ostream& operator<<(std::ostream& strm, const ShowX509& cert) {
    if(cert.cert) {
        ...
        assert(name);
        owned_ptr<BIO> io(__FILE__, __LINE__, BIO_new(BIO_s_mem()));
