...
struct PKCS12Writer {
    ...
    void write(const char* fname,
        ...
        owned_ptr<PKCS12> p12(PKCS12_create(passwd,
                                            friendlyName,
                                            key,
                                            cert,
                                            cacerts.get(),
...
struct CertCreator {
    ...
    std::tuple<owned_ptr<EVP_PKEY>, owned_ptr<X509>> create()
    {
        ...
        add_extension(cert.get(), NID_basic_constraints, isCA ? "CA:TRUE" : "CA:FALSE");
...
int main(int argc, char *argv[])
{
    try {
        ...
        // extendedKeyUsage derivied from name: client, server, or IOC (both client and server)
