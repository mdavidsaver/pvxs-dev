...
void qsrv2Init(initHookState hook)
{
    if(hook!=initHookAfterIocBuilt)
        return;

    pvxs::ioc::server()
            .addSource("qsrv", std::make_shared<DBSingleSource>());

}

void qsrv2Reg(void)
{
    initHookRegister(&qsrv2Init);
}

} // namespace

extern "C" {
epicsExportRegistrar(qsrv2Reg);
// in .dbd file
//registrar(qsrv2Reg)
}
